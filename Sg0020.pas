...
type
  TSg_0020 = class(TForm)
    ...
    Label18: TLabel;
    Label19: TLabel;
    DBEdit_Profissao: TDBEdit;
    Label20: TLabel;
    DBCB_sexo: TDBComboBox;
    StatusBar1: TStatusBar;
    DBRG_boleto: TDBRadioGroup;
    Panel1: TPanel;
    Label21: TLabel;
    DBT_codigo: TDBText;
    sSpeedButton1: TsSpeedButton;
    sSpeedButton2: TsSpeedButton;
    DBEdit_COD_CONV: TDBEdit;
    SB_Convenio: TsSpeedButton;
    ...
    procedure SB_ConvenioClick(Sender: TObject);
  private
    ComponAnt : TDBEdit;
...
procedure TSg_0020.ControlChange(Sender : TObject);
begin
...
procedure TSg_0020.Manutencao(Botao: Integer; Tabela: TIBDataSet);
var cod : integer;
begin
  case Botao of
    // Incluir
    1 : begin
          ...
          if bDlg_Inc = True then
             Tabela.FieldByName('NOME').AsString      := Dlg_Socio.Edit_pesquisa.Text;
          Tabela.FieldByName('FLG_TIPO').AsString     := 'F';
          Tabela.FieldByName('FLG_ATIVO').AsString    := 'A';
          Tabela.FieldByName('FLG_BOLETO').AsString   := 'N';
          Tabela.FieldByName('SEXO').AsString         := 'MASCULINO';
          Tabela.FieldByName('VALOR').AsFloat         := 0;
    ...
    7 : begin
          ...
          if Dlg_Socio.ShowModal = mrOk then
            begin
               Tabela.Close;
               Tabela.ParamByName('CODIGO').AsInteger := Dlg_Socio.SQL_Pesquisa.FieldByName('CODIGO').AsInteger;
               Tabela.Open;
               Ativa_Source;
            end;
        end;
...
procedure TSg_0020.SB_ConvenioClick(Sender: TObject);
begin
  Dlg_Empresa.Edit_pesquisa.Clear;
  if Dlg_Empresa.ShowModal = mrOk then
    begin
       DBEdit_Empresa.Text  := Dlg_Empresa.SQL_Pesquisa.FieldByName('NOME').AsString;
       DBEdit_COD_CONV.Text := FormatFloat('000000',Dlg_Empresa.SQL_Pesquisa.FieldByName('CODIGO').AsInteger);
    end;
end;

procedure TSg_0020.sSpeedButton1Click(Sender: TObject);
begin
//BOTÃO ESCONDIDO PARA CADASTRO DAS EMPRESAS JÁ CADASTRADAS ANTERIORMENTE
  ...
  Dm.IBT_SgEdu.CommitRetaining;

  with Sg_0000.IBQ_PesqSocio, SQL do
    begin
      Close;
      Clear;
      Add('update socio set empresa = ''GLAUBER RODRIGUES MADEIRAS''');
      Add('where (empresa like ''MADE%'')');
      Open;
    end;
  ...
  with Sg_0000.IBQ_PesqSocio, SQL do
    begin
      Close;
      Clear;
      Add('update socio set empresa = ''SOLARE''');
      Add('where (empresa like ''SOLARE%'')');
      Open;
    end;
  ...
  with Dm.IBQ_Pesquisa, SQL do
    begin
      Close;
      Clear;
      Add('select empresa from socio group by empresa');
      Open;
      First;
    end;
  ...
  if Dm.IBQ_Pesquisa.RecordCount > 0 then
    begin
      while not Dm.IBQ_Pesquisa.Eof do
        begin
          if Dm.IBQ_Pesquisa.FieldByName('EMPRESA').AsString = '' then Dm.IBQ_Pesquisa.Next;

          with Sg_0000.IBQ_PesqSocio, SQL do
            begin
              Close;
              Clear;
              Add('INSERT INTO CONVENIO (CODIGO, NOME, FLG_ATIVO) VALUES (:CODIGO, :NOME, :FLG_ATIVO)');
              ParamByName('CODIGO').AsInteger := 0;
              ParamByName('NOME').AsString := Dm.IBQ_Pesquisa.FieldByName('EMPRESA').AsString;
              ParamByName('FLG_ATIVO').AsString := 'A';
              Open;
            end;
          Dm.IBT_SgEdu.CommitRetaining;
...
procedure TSg_0020.sSpeedButton2Click(Sender: TObject);
begin
  with Dm.IBQ_Pesquisa, SQL do
    begin
      Close;
      Clear;
      Add('Select codigo, nome from convenio');
      Open;
      First;
    end;
    ...
    begin
      with Dm.IBQ_PesqAux, SQL do
        begin
          Close;
          Clear;
          Add('Update socio set cod_conv = :codigo where empresa = :empresa');
          ParamByName('CODIGO').AsInteger := Dm.IBQ_Pesquisa.FieldByName('CODIGO').AsInteger;
          ParamByName('EMPRESA').AsString := Dm.IBQ_Pesquisa.FieldByName('NOME').AsString;
          Open;
        end;
      Dm.IBQ_Pesquisa.Next
    end;
...
procedure TSg_0020.Bbtn_IncluirClick(Sender: TObject);
begin
  Manutencao((Sender as TBitBtn).Tag, Dm.IBDS_Socio);
end;

procedure TSg_0020.FormShow(Sender: TObject);
begin
...
procedure TSg_0020.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
procedure TSg_0020.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TSg_0020.StateChange(Sender: TObject);
begin
...
procedure TSg_0020.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Dm.DS_Socio.OnStateChange := nil;
  Dm.IBDS_Socio.close;
end;

procedure TSg_0020.FormDestroy(Sender: TObject);
begin
   Screen.OnActiveControlChange := Nil;
end;

procedure TSg_0020.DBRG_pessoaExit(Sender: TObject);
begin
...
procedure TSg_0020.DBEdit_cnpjKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var snum : String;
begin
...
procedure TSg_0020.DBEdit_dt_nascKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TSg_0020.DBEdit_dt_nascEnter(Sender: TObject);
begin
    Dm.IBDS_Socio.fieldByName('DT_NASC').EditMask := '99/99/9999;1';
end;

procedure TSg_0020.Ativa_Source;
begin
   ...
   DBEdit_Cod_CONV.DataSource     := Dm.DS_Socio;
   DBEdit_Cod_CONV.DataField      := 'COD_CONV';
